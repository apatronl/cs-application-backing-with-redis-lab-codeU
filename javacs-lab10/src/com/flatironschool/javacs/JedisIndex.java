...
public class JedisIndex {
	...
	public Map<String, Integer> getCounts(String term) {
		...
		for (String url : urls) {
			String redisKey = termCounterKey(url);
			if (jedis.hexists(redisKey, term)) {
				counts.put(url, Integer.parseInt(jedis.hget(redisKey, term)));
			} else {
				counts.put(url, 0);
			}
		}
	...
	public Integer getCount(String url, String term) {
		String redisKey = termCounterKey(url);
		return jedis.hexists(redisKey, term) ? Integer.parseInt(jedis.hget(redisKey, term)) : 0;
	}
	...
	public void indexPage(String url, Elements paragraphs) {
		...
		for (String key : tc.keySet()) {
			...
			t.hset(termCounterKey, key, Integer.toString(tc.get(key)));
		}
		t.exec();
	}
